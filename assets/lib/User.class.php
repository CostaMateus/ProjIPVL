<?php
	...
	class User {
		...
		private function qSelectUserDefault() {
			try{ 
				$lg = "default";
				$stmt = $this->conn->open_db()->prepare("SELECT `pass` FROM `users` WHERE `login` = :login;");
				$stmt->bindParam(":login", $lg, PDO::PARAM_STR);
		...
		public function qInsert($data) {
			try {
				...
				$this->pass = password_hash($data['pass1'], PASSWORD_BCRYPT);
		...
		public function validaLogin($data) {
			...
			$this->pass = $data['pass'];
			try {
				$default = $this->qSelectUserDefault();
				$result = $this->qSelect(null, $this->login);
				if (($result != null) && (password_verify($this->pass, $default['pass']))) {
					header('location: ' . BASEURL . 'admin/acesso.php?id=' . $this->objFunc->base64($result['idUser'], 1));
				} elseif (($result != null) && (password_verify($this->pass, $result['pass']))) {
					if($result['active'] == 1) {
						session_start();
						...
						header('location: ' . BASEURL . 'admin/view/?=' . $default['pass']);
					} else {
